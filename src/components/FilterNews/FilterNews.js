...
const FilterNews = (props) => {
    ...
    const fetchFilterData = () => {
        try {
            HttpService.get("news/filters", "", true).then((response) => {
                const parsedData = response.data.data;
                setCategories(parsedData.categories);
                setAuthors(parsedData.authors);
                const preferredSources =
                    parsedData.preferred_sources.split(",");
                const preferredCategories =
                    parsedData.preferred_categories.split(",");
                const preferredAuthors =
                    parsedData.preferred_authors.split(",");
                    ...
                    <Col sm={12} md={3} lg={3} xl={3}>
                        <FormGroup controlId="formGridSource">
                            ...
                            >
                                <option
                                    value="1"
                                    selected={filterSources.includes("1")}
                                >
                                    NewsAPI
                                </option>
                                <option
                                    value="2"
                                    selected={filterSources.includes("2")}
                                >
                                    Guardian
                                </option>
                                <option
                                    value="3"
                                    selected={filterSources.includes("3")}
                                >
                                    New York Times
                                </option>
                            </FormControl>
                        ...
                        >
                            {categories.map((category) => (
                                <option
                                    key={category}
                                    value={category}
                                    selected={filterCategories.includes(
                                        category
                                    )}
                                >
                                    {category}
