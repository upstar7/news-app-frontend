      ...
      if (response.data.status.error) {
        if (Object.keys(response.data.status.validation_errors).length > 0) {
          ...
          if (response.data.status.validation_errors.name) {
            setNameErrors(response.data.status.validation_errors.name);
          }
          if (response.data.status.validation_errors.email) {
            setEmailErrors(response.data.status.validation_errors.email);
          }
          if (response.data.status.validation_errors.password) {
            setPasswordErrors(response.data.status.validation_errors.password);
          }
        } else {
          setErrorMessage(response.data.status.message);
        }
